...
According to our calculations (and ignoring margin of error for now), the average unemployment rate in these tracts between 2007 and 2013 was estmated to be `r round(avg_2013,1)` percent. Between 2013 and 2015, that rate changed to `r round(avg_2015,1)` percent. That's a difference of `r round(avg_diff,1)`. 
...
# 1 Journal Square (2014)

**County:** Hudson

**Census tracts:** 19, 20, 31, 44, 45, 46, 49, 52, 53, 58.01, 60, 67, 68

**Address:** 1 Journal Square, Jersey City, NJ 07306

```{r methodology2, fig.width=9, fig.height=6, message=F, warning=F}

#* A ratio was developed for the census tracts based on the ACS 2007 - 2011 Census data and then applied to the 2013 annual average data for the derivation county.

#* The ACS 2007 - 2011 Census employment and unemployment figures for the tract are divided by the corresponding ACS 2007-2011 Census employment and unemployment data for the derivation county.

#* The resulting ratios are then aplied to the 2013 annual average employment and unemployment data for the derivation county

#* These unemployment and unemployment figures are then summed to determine the labor force figure

#* Finally, the unemployment rate is calculated by dividing the unemployment figure by the labor force figure

#* The qualifying rate using 2013 annual average data is 11.1 percent (1.5 times the US not-seasonally-adjusted unemployment rate for the same time period)
```

There are 164 census tracts within Hudson County. 

The tracts associated with this project include the areas that have the highest unemployment rates in the county. The tract where 1 Journal Square was constructed tied for 18th with the lowest unemployment rate at 4 percent in 2011. Tract 46 was included by planners within the project borders but is nearly two and a half miles away from the site and had an unemployment rate of 23 percent— the second-worst rate in Hudson county.

According to the application, the combined unemployment rate is greater than or equal to the qualifying rate of 15.6 percent (150 percent of the US rate for the same time period).


```{r data_prep2, warning=F, message=F}
# GEOIDs: 34017001900, 34017002000, 34017003100, 34017004400, 34017004500, 34017004600, 34017004900, 34017005200, 34017005300, 34017005500, 34017005801, 34017006000, 34017006700, 34017006800
...
proj1 <- c("34017001900", "34017002000", "34017003100", "34017004400", "34017004500", "34017004600", "34017004900", "34017005200", "34017005300", "34017005500", "34017005801", "34017006000", "34017006700", "34017006800")
...
hudson_unemp2 <- gather(hudson_unemp2, "year", "unemployment_rate", 3:4)
hudson_unemp3 <- select(hudson_unemp, tract=name, `unemployment rate 2011`=ur_2011, `unemployment rate 2015`=ur_2015, `2011 rank`=`2011_rank`, `2015 rank`=`2015_rank`)
hudson_unemp3$tract <- gsub(",.*", "", hudson_unemp3$tract)
datatable(hudson_unemp3)
```

According to our calculations (and ignoring margin of error for now), the average unemployment rate in these tracts between 2008 and 2011 was estmated to be `r round(avg_2011,1)` percent. Between 2011 and 2015, that rate changed to `r round(avg_2015,1)` percent. That's a difference of `r round(avg_diff,1)`. 


```{r double_check2, fig.width=9, fig.height=6, message=F, warning=F}
# bringing in jersey city tract boundaries
...
nj_map <- nj_map + scale_color_viridis(option = "heat", direction=-1)
nj_map <- nj_map + theme_nothing(legend=TRUE) 
nj_map <- nj_map + labs(x=NULL, y=NULL, title="1 Journal Square (2014)")
nj_map <- nj_map + theme(panel.grid.major = element_line(colour = NA))
nj_map <- nj_map + theme(text = element_text(size=15))
nj_map <- nj_map + theme(plot.title=element_text(face="bold", hjust=.4))
nj_map <- nj_map + theme(plot.subtitle=element_text(face="italic", size=9, margin=margin(l=20)))
...
sp <- sp+ theme(legend.position = "none", 
                ...
                axis.text.y = element_blank(), 
                plot.title = element_text(size = 18))  
sp <- sp + ggtitle("Unemployment rate in tracts associated with 1 Journal Square (2014)")
sp <- sp + xlab("")
print(sp)
```

```{r map2, fig.width=9, fig.height=6, warning=F, message=F}
hudson_unemp4 <- subset(hudson_unemp2, year=="2011")
nj_merged <- geo_join(nj_h, hudson_unemp4, "GEOID", "id")
